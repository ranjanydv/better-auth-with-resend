...
export const Sidebar = ({ isCollapsed }: { isCollapsed: boolean }) => {
  return (
    <aside
      className={`bg-white border-r border-border h-screen transition-all duration-300 fixed left-0 top-0 z-0 ${
    ...
    >
      <div className="p-4 py-2 border-b border-border">
        <div className="flex items-center h-12">
          ...
          {!isCollapsed && (
            <span className="ml-3 font-bold text-primary text-2xl">
              Nexsus
            </span>
          )}
        </div>
      ...
      <nav className="space-y-6 mt-4 px-2">
        {navigationData.map((section) => (
          <div key={section.title} className="space-y-2">
            {!isCollapsed && (
              <h3 className="px-4 font-semibold text-primary text-xs uppercase tracking-wider">
                {section.title}
              </h3>
            )}
